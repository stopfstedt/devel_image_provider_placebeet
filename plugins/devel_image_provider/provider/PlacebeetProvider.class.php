...
/**
 * @file
 * Placebeet support class.
 */

/**
 * Add support for placebeet.com.
 */
class PlacebeetProvider extends DevelImagesProviderBase {

    public function __construct($plugin) {
        parent::__construct($plugin);
        $this->provider_base_url = 'http://placebeet.com';
    }
    ...
    public function settingsForm() {
        ...
        $form['devel_image_provider_' . $this->plugin['name']]['devel_image_provider_watermark'] = array(
          '#type' => 'checkbox',
          '#title' => t('Overlay text with image dimensions'),
          '#default_value' => isset($this->settings['devel_image_provider_watermark']) ? $this->settings['devel_image_provider_watermark'] : NULL,
        );
    ...
    public function generateImage($object, $field, $instance, $bundle) {
        ...
        if (!isset($images[$extension][$min_resolution][$max_resolution]) || count($images[$extension][$min_resolution][$max_resolution]) <= DEVEL_GENERATE_IMAGE_MAX) {
            if ($tmp_file = drupal_tempnam('temporary://', 'imagefield_')) {
                ...
                $watermark = isset($this->settings['devel_image_provider_watermark']) ? $this->settings['devel_image_provider_watermark'] : NULL;

                $watermark_part = $watermark ? '/d' : '';

                $url = $this->provider_base_url . $watermark_part . "/$width/$height" . $gray_part;
